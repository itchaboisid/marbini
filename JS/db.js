const questions = [
  {
    id: 1,
    question:
      "What kind of hobbies do you enjoy in your daily life basis?",
    answers: [
      {
        text: "Gaming / Any computer activities",
        cat: "abacus",
      },
      {
        text: "Dancing / Singing / Arts",
        cat: "likha",
      },
      {
        text: "Basketball / Volleyball / Badminton",
        cat: "sports",
      },
      {
        text: "Pictures / Writing",
        cat: "letrato",
      },
    ],
  },
  {
    id: 2,
    question:
    "Which subject do you prefer in these choices?",
    answers: [
      {
        text: "Science",
        cat: "science",
      },
      {
        text: "Math",
        cat: "math",
      },
      {
        text: "Mapeh",
        cat: "likha",
      },
      {
        text: "ESP",
        cat: "ambassadors",
      },
    ],
  },
  {
    id: 3,
    question:
    "What kind of hobbies do you enjoy in your daily life basis?(2)",
    answers: [
      {
        text: "Experiments",
        cat: "science",
      },
      {
        text: "Learning Computations and Formulas",
        cat: "math",
      },
      {
        text: "Coding ",
        cat: "abacus",
      },
      {
        text: "Helping out other people",
        cat: "ambassadors",
      },
    ],
  },
  {
    id: 4,
    question:
    "What aspect of club involvement appeals to you the most?",
    answers: [
      {
        text: "Engaging in hands-on technical projects and problem-solving",
        cat: "abacus",
      },
      {
        text: "Expressing yourself creatively through performances or artistic creations",
        cat: "likha",
      },
      {
        text: "Participating in competitive sports events and fostering team spirit",
        cat: "sports",
      },
      {
        text: "Immersing yourself in visual storytelling and capturing memorable moments",
        cat: "letrato",
      },
    ],
  },
  {
    id: 5,
    question:
    "Which subject do you prefer in these choices?(2)",
    answers: [
      {
        text: "TLE (Computer)",
        cat: "abacus",
      },
      {
        text: "Physical Education",
        cat: "sports",
      },
      {
        text: "Araling Panlipunan",
        cat: "ambassadors",
      },
      {
        text: "Filipino",
        cat: "letrato",
      },
    ],
  },
  {
    id: 6,
    question:
    "How do you cope up with stress?",
    answers: [
      {
        text: "Playing sports",
        cat: "sports",
      },
      {
        text: "Dancing/Singing",
        cat: "likha",
      },
      {
        text: "Poetry",
        cat: "letrato",
      },
      {
        text: "Solving Math Problem",
        cat: "math",
      },
    ],
  },
  {
    id: 7,
    question:
    "How do you cope up with stress?(2) ",
    answers: [
      {
        text: "Playing online games",
        cat: "abacus",
      },
      {
        text: "Helping others",
        cat: "ambassador",
      },
      {
        text: "Experiment on things",
        cat: "science",
      },
      {
        text: "Playing sports",
        cat: "sports",
      },
    ],
  },
  {
    id: 8,
    question:
    "What is a new experience you've always wanted to try?",
    answers: [
      {
        text: "Learn physical activities",
        cat: "sports",
      },
      {
        text: "Learn how to do experiments",
        cat: "science",
      },
      {
        text: "Get knowledge in mathematics",
        cat: "math",
      },
      {
        text: "Remembering events using pictures",
        cat: "letrato",
      },
    ],
  },
  {
    id: 9,
    question:
    "What is a new experience you've always wanted to try?",
    answers: [
      {
        text: "Play competitive basketball / Volleyball",
        cat: "sports",
      },
      {
        text: "Making a game",
        cat: "abacus",
      },
      {
        text: "Perform in a stage",
        cat: "likha",
      },
      {
        text: "Communicate and help other students",
        cat: "ambassadors",
      },
    ],
  },
  {
    id: 10,
    question:
    "What aspect of club involvement appeals to you the most?",
    answers: [
      {
        text: "Helping other people / Volunteer",
        cat: "ambassador",
      },
      {
        text: "Solving math problems",
        cat: "math",
      },
      {
        text: "Experimenting new things",
        cat: "science",
      },
      {
        text: "Playing online games",
        cat: "abacus",
      },
    ],
  },
];

module.exports = questions;